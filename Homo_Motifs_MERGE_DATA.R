...
# Visits_tot,	homo_motif,	hete_motif
...
homo_table <- as_tibble(expand.grid(Plot=1:9,Subplot=Subplot_list,
...
for(i in 1:nrow(homo_table)){
  ...
  h_m_i <- data_real %>% filter(Plot==homo_table$Plot[i],
                                G_F==homo_table$G_F[i],
                                Subplot_Plant_Label==paste(homo_table$Subplot[i],
                                                           homo_table$Plant_Label[i],sep=" "))
  
  if (length(h_m_i$homo_motif)!=0){
    homo_table$motifs_Caracoles[i] <- as.numeric(h_m_i$homo_motif)}else{
      homo_table$motifs_Caracoles[i] <- 0
    }
...
autofill_homo_motifs <- function(list_rd_files,data_table){
  
  #data_table_or <- homo_table #TEST
  ...
  for(i in 1:length(files_simpl)){
    ...
    for (j in 1:nrow(data_table)){
      ...
      if (length(h_m_i$homo_motif)!=0){
        data_table[j,initial_number_columns+i] <- as.numeric(h_m_i$homo_motif)
        cont=cont+1}else{
...
Homo_data_subplot <- autofill_homo_motifs(list_rd_files,homo_table)

write_csv(Homo_data_subplot, paste("Total_Homo_data_subplot.csv",sep = ""))
