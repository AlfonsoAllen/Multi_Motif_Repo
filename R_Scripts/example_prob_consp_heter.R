...
for (Plot_i in list_sites){
  ...
  stationary_transition_matrix <- 0.5*(transpose_transition_matrix %^% 1000000+transpose_transition_matrix %^% 1000001)
  ...
  for (i in 1:nrow(stationary_probabilities_consp_heter)) {
    ...
    stationary_probabilities_consp_heter$consp_prob[i] <- 
      sum(stationary_transition_matrix_plant[consp_rows,i])
