...
fitness.data.zeros <- fitness_orig
...
fitness_LEMA_ZI <- subset(fitness.data.zeros,Plant_Simple == "LEMA")
fitness_PUPA_ZI <- subset(fitness.data.zeros,Plant_Simple == "PUPA")
fitness_CHFU_ZI <- subset(fitness.data.zeros,Plant_Simple == "CHFU")
...
GF_all_sp <- glmmTMB(Seeds_GF ~ scale(homo_motif) + 
                       scale(hete_motif) + 
                       scale(DegreeIn) + 
                       (1 + scale(homo_motif) + scale(hete_motif) | Plant_Simple) + 
                       (1 + scale(homo_motif) + scale(hete_motif) | ID),
                     family = nbinom2(),
                     data = fitness.data)
...
GF_LEMA_01_ZI <- glmmTMB(Seeds_GF ~ scale(homo_motif) + 
                        scale(hete_motif) + 
                        scale(DegreeIn) + 
                        (1|ID),
                      ziformula = ~1,
                      family = nbinom2(),
                      data = fitness_LEMA_ZI)

GF_PUPA_01_ZI <- glmmTMB(Seeds_GF ~ scale(homo_motif) + 
                        # scale(hete_motif) + 
                        scale(DegreeIn) + 
                        (1|ID),
                      ziformula = ~1,
                      family = nbinom2(),
                      data = fitness_PUPA_ZI)
...
GF_LEMA_b <- brm(Seeds_GF ~ (1 + scale(homo_motif) + 
                   scale(hete_motif) + 
                   scale(DegreeIn) |ID),
...
get_variables(GF_LEMA_b)

tt <- GF_LEMA_b %>% spread_draws(b_Intercept,r_ID[Bees,scalehomo_motif])
