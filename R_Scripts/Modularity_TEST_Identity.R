...
# This script test if the modules identified by infomap change significantly along new realizations 
...
# Generate seeds to initialize infomap
seeds_ini <- c(497294)
seeds <- sample(1:497293, n_replicates)
seeds_ini <- c(seeds_ini,seeds)
...
for (i in 1:length(list_files_field_level)){
  ...
  for (k in 1:length(seeds_ini)){
  
    modules_relax_rate <- run_infomap_multilayer2(Plot_multilayer, relax = F, silent = T, flow_model = 'directed', trials = 1000, seed = seeds_ini[k], temporal_network = F)
  
    if (k==1){
      results_i <- modules_relax_rate$modules
      col_names <- c("node_id","layer_id","type","species","module")
      results_i <- results_i[,col_names]
      names(results_i)[5:ncol(results_i)] <- 1:k
    }else{
      results_i <- results_i %>% left_join(modules_relax_rate$modules,by=c("node_id", "layer_id", "type",  "species"))
      names(results_i)[5:ncol(results_i)] <- 1:k
    }
...
for (i in 6:ncol(results)){
  check_column <- all(results[,5] == results[,i])
  comparison <- c(comparison,check_column)
}

all(comparison)


# It seems that changing the seed do not alter the optimal solution found by infomap