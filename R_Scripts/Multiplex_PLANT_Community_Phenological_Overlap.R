...
pollination_19_i <- pollination_19_i %>% select(Day,Month,Year,Plot,Subplot,Plant_Simple,G_F,Visits) %>%
...
plant_pheno_overlap <- function(plant1,plant2,pollination_19_i) {
  ...
  if(sum(c(plant1,plant2) %in% plants)<2){
    ...
    total_weeks_i <-  max(pollination_19_i$Week) - (min(pollination_19_i$Week)-1)
    
    return(overlap/total_weeks_i)
    #return(overlap)
...
folder_base <- paste(dir_ini,"/Processed_data/Multilayer/",sep="")
  ...
  if (length(polinator_layers)>1){
    ...
    for (j in 1:nrow(combination_layers)){
      interlink_i<- tibble(layer_from=combination_layers[j,1],
                           ...
                           weight=plant_pheno_overlap(combination_layers[j,1],
...
plot_modules %>% filter(module==1,type!="plant")
...
ggplot(plot_modules,
  ...
  labs(x='Plant Layer', y='Number of species',
       title = paste("Plot ",Plot_i,': Modules across plant layers (Phenological Overlap)',sep=""))+
