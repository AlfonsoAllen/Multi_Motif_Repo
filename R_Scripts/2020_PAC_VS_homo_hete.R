...
fitness2 %>% filter(Week==21) %>% group_by(Plot,Week) %>% count(wt=Visits) %>% 
  arrange(desc(n))
Week_i <- 21
Plot_i <- 8
example_plot_8_week_21 <- fitness2 %>% filter(Week==Week_i,Plot==Plot_i)


for (week_i in unique(example_plot_8_week_21$Week)){
  
  print("WEEK")
  print(week_i)
  
  
  fitness_week_i <- example_plot_8_week_21 %>% filter(Week==week_i)
  ...
  if (week_i==min(example_plot_8_week_21$Week)){
...
motifs_example <- visit_list
motifs_example %>% select(-Plot, -ID, -Week) %>% group_by(Subplot_Plant_Label) %>%
  summarize_all(sum) 
###########################
# PAC example

library(bipartite)
PAC_example_plot_8_week_21 <- example_plot_8_week_21 %>% 
  mutate(Subplot_Plant = paste(Subplot,Plant,sep=" ")) %>%
  group_by(Subplot_Plant,ID) %>% count(wt=Visits) %>% spread(ID,n)

PAC_example_plot_8_week_21[is.na(PAC_example_plot_8_week_21)] <- 0

PAC_example_matrix <- PAC_example_plot_8_week_21 %>% ungroup() %>%
  select(-Subplot_Plant) %>% as.matrix()
rownames(PAC_example_matrix) <- PAC_example_plot_8_week_21$Subplot_Plant 

PAC(PAC_example_matrix) %>% as_tibble()
plotweb(PAC_example_matrix)
