...
#############################
# CORRECTING CENTRALITY

PageRank_results_ini <- centrality_final
...
PageRank_results_Plant_Simple <- PageRank_results_ini %>% mutate(Label=species) %>% 
  separate(Label,c("aux","Plant_Simple")," ") 


# Fix problems with those visitors names with spaces
PageRank_results_Plant_Simple$species[grep("Lassioglosum_.immunitum",
                                           PageRank_results_Plant_Simple$species)] <- 
  gsub("Lassioglosum_.immunitum", 
       "Lassioglosum_ immunitum", 
       PageRank_results_Plant_Simple$species[grep("Lassioglosum_.immunitum",
                                                  PageRank_results_Plant_Simple$species)])
...
PageRank_results <- PageRank_results_Plant_Simple %>% 
  dplyr::select(species,C_Real_PR_Multi,C_Real_PR_Layer,
                StrengthIn,StrengthOut,DegreeIn,DegreeOut,
                C_Delta,C_Ratio,Plot) %>%
  rename(Real_PR_Multi=C_Real_PR_Multi,
         Real_PR_Layer=C_Real_PR_Layer,
         Delta=C_Delta,
         Ratio=C_Ratio)

# Sanity Check

all(names(centrality_final)==names(PageRank_results))

write_csv(PageRank_results,"NN_C_PageRank_results.csv")
