...
pollination_19_i <- pollination %>% filter(Year==2019,Subplot!="OUT",!is.na(ID))

pollination_19_i <- pollination_19_i %>% dplyr::select(Day,Month,Year,Line,Plot,Subplot,Plant_Simple,ID,Visits) %>%
...
plant_pheno_overlap <- function(plant1,plant2,pollination_19_i) {
  ...
  if(sum(c(plant1,plant2) %in% plantsx)<2){
    print(paste("Error: At least one plant does not belong to Caracoles experimental phenology",sep=""))
    }else{
...
###########################
# CREATE MULTILAYER FOR Caracoles_i
###########################
...
# Extract layer files for Caracoles

list_files_field_level <- files_base[grepl(paste("Caracoles_",sep = ""), files_base)]
...
write_csv(plot_modules_i,"Processed_data/Modularity_Pheno_Overlap/Modularity_Caracoles.csv")


modules_plants_caracoles <-  plot_modules_i %>% filter(type=="plant") %>%
  separate(species,c("Plot","Subplot","Plant_Simple")," ")
  
modules_plants_caracoles$Line <- NA

for (i in 1:nrow(modules_plants_caracoles)){
  if(modules_plants_caracoles$Plot[i] %in% c(1,2,3)){modules_plants_caracoles$Line[i] <- 1}
  else if(modules_plants_caracoles$Plot[i] %in% c(4,5,6)){modules_plants_caracoles$Line[i] <- 2}
  else{modules_plants_caracoles$Line[i] <- 3}
}

modules_plants_caracoles$Plot <- as.factor(modules_plants_caracoles$Plot)
modules_plants_caracoles$Plant_Simple <- as.factor(modules_plants_caracoles$Plant_Simple)
modules_plants_caracoles$Line <- as.factor(modules_plants_caracoles$Line)

modules_plants_caracoles_agg <- modules_plants_caracoles %>%
  group_by(module, Plant_Simple,Line)%>% count()

ggplot(modules_plants_caracoles_agg)+
  geom_point(aes(x=module,y=Plant_Simple,color=Plant_Simple,size=n))+
  facet_wrap(vars(Line),nrow = 3,ncol = 1)

modules_lines_agg <- modules_plants_caracoles_agg %>%
  group_by(module, Line)%>% count(wt=n)
