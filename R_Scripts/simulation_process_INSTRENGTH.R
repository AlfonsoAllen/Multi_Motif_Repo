...
summary_InStr_intralinks <- total_InStr_intralinks_CENTRALITY_deco %>%
  group_by(Plant,number_intra,number_inter,number_individuals,number_pollinators) %>% 
  summarise(Mean = mean(StrengthIn, na.rm = T), SD = sd(StrengthIn, na.rm = T))

summary_InStr_intralinks$Plant[summary_InStr_intralinks$Plant=="plant_sp_1"] <- "Sp. 1"
summary_InStr_intralinks$Plant[summary_InStr_intralinks$Plant=="plant_sp_2"] <- "Sp. 2"
summary_InStr_intralinks$Plant[summary_InStr_intralinks$Plant=="plant_sp_3"] <- "Sp. 3"
summary_InStr_intralinks$Plant[summary_InStr_intralinks$Plant=="plant_sp_4"] <- "Sp. 4"
summary_InStr_intralinks$Plant[summary_InStr_intralinks$Plant=="plant_sp_5"] <- "Sp. 5"

summary_InStr_interlinks <- total_InStr_interlinks_CENTRALITY_deco %>%
  group_by(Plant,number_intra,number_inter,number_individuals,number_pollinators) %>% 
  summarise(Mean = mean(StrengthIn, na.rm = T), SD = sd(StrengthIn, na.rm = T))

summary_InStr_interlinks$Plant[summary_InStr_interlinks$Plant=="plant_sp_1"] <- "Sp. 1"
summary_InStr_interlinks$Plant[summary_InStr_interlinks$Plant=="plant_sp_2"] <- "Sp. 2"
summary_InStr_interlinks$Plant[summary_InStr_interlinks$Plant=="plant_sp_3"] <- "Sp. 3"
summary_InStr_interlinks$Plant[summary_InStr_interlinks$Plant=="plant_sp_4"] <- "Sp. 4"
summary_InStr_interlinks$Plant[summary_InStr_interlinks$Plant=="plant_sp_5"] <- "Sp. 5"

summary_InStr_individuals <- total_InStr_individuals_CENTRALITY_deco %>%
  group_by(Plant,number_intra,number_inter,number_individuals,number_pollinators) %>% 
  summarise(Mean = mean(StrengthIn, na.rm = T), SD = sd(StrengthIn, na.rm = T))

summary_InStr_individuals$Plant[summary_InStr_individuals$Plant=="plant_sp_1"] <- "Sp. 1"
summary_InStr_individuals$Plant[summary_InStr_individuals$Plant=="plant_sp_2"] <- "Sp. 2"
summary_InStr_individuals$Plant[summary_InStr_individuals$Plant=="plant_sp_3"] <- "Sp. 3"
summary_InStr_individuals$Plant[summary_InStr_individuals$Plant=="plant_sp_4"] <- "Sp. 4"
summary_InStr_individuals$Plant[summary_InStr_individuals$Plant=="plant_sp_5"] <- "Sp. 5"

summary_InStr_pollinators <- total_InStr_pollinators_CENTRALITY_deco %>%
  group_by(Plant,number_intra,number_inter,number_individuals,number_pollinators) %>% 
  summarise(Mean = mean(StrengthIn, na.rm = T), SD = sd(StrengthIn, na.rm = T))

summary_InStr_pollinators$Plant[summary_InStr_pollinators$Plant=="plant_sp_1"] <- "Sp. 1"
summary_InStr_pollinators$Plant[summary_InStr_pollinators$Plant=="plant_sp_2"] <- "Sp. 2"
summary_InStr_pollinators$Plant[summary_InStr_pollinators$Plant=="plant_sp_3"] <- "Sp. 3"
summary_InStr_pollinators$Plant[summary_InStr_pollinators$Plant=="plant_sp_4"] <- "Sp. 4"
summary_InStr_pollinators$Plant[summary_InStr_pollinators$Plant=="plant_sp_5"] <- "Sp. 5"

p_intra <- ggplot(summary_InStr_intralinks, aes(x=Plant, y=Mean, fill=Plant))+
  ...
  labs(title="Dependence of the number of in-strength on the number of intralinks\n(30 interlinks, 50 plant individuals, 10 pollinator sps.)")
  
p_inter <- ggplot(summary_InStr_interlinks, aes(x=Plant, y=Mean, fill=Plant))+facet_wrap(~number_inter)+
  geom_bar(stat = "identity", color = "black", position = position_dodge())+
  geom_errorbar(aes(ymin = Mean, ymax = Mean+SD),width = 0.2,
                position = position_dodge(0.9))+
  scale_fill_brewer(palette = "Paired")+theme_bw()+
  labs(title="Dependence of the number of in-strength on the number of interlinks\n(125 intralinks, 50 plant individuals, 10 pollinator sps.)")

p_indiv <- ggplot(summary_InStr_individuals, aes(x=Plant, y=Mean, fill=Plant))+facet_wrap(~number_individuals)+
  geom_bar(stat = "identity", color = "black", position = position_dodge())+
  geom_errorbar(aes(ymin = Mean, ymax = Mean+SD),width = 0.2,
                position = position_dodge(0.9))+
  scale_fill_brewer(palette = "Paired")+theme_bw()+
  labs(title="Dependence of the number of in-strength on the total number of plant individuals\n(125 intralinks, 30 interlinks, 10 pollinator sps.)")

p_polli <- ggplot(summary_InStr_pollinators, aes(x=Plant, y=Mean, fill=Plant))+
  facet_wrap(~number_pollinators)+
  geom_bar(stat = "identity", color = "black", position = position_dodge())+
  geom_errorbar(aes(ymin = Mean, ymax = Mean+SD),width = 0.2,
                position = position_dodge(0.9))+
  scale_fill_brewer(palette = "Paired")+theme_bw()+
  labs(title="Dependence of the number of in-strength on the total number of pollinator sp.\n(125 intralinks, 30 interlinks, 50 plant individuals)")



png("New_Figures/simulations_INSTRENGTH.png",
