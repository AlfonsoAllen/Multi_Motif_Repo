...
data_simulation_raw <- read_csv("Processed_data/Data_simulation/data_changing_interlinks.csv")

list_plots <- data_simulation_raw$Plot %>% unique()

data_changing_interlinks_MOTIFS <- NULL
...
for(i.plot in 1:length(list_plots)){
  ...
  data_changing_interlinks_MOTIFS <- bind_rows(data_changing_interlinks_MOTIFS,
                                               aggregate_total_plot)
  
  # Commented for security reasons
  # write_csv(data_changing_interlinks_MOTIFS,
  #           "Processed_data/Data_simulation/data_changing_interlinks_MOTIFS.csv")
