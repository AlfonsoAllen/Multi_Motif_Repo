...
#NOTICE THAT some Plots may have no interlinks (for instance Plot 4)
...
#Filter pollination datafilter
pollination_20_i <- pollination %>% filter(!is.na(Plant),Plant!="0",Plant!="Ground",Subplot!="OUT",Plot==Plot_i,!is.na(ID))
...
pollination_20_i <- pollination_20_i %>% select(Day,Month,Year,Line,Plot,Subplot,Plant,ID_Simple,Visits) %>%
  mutate(date_raw=as.Date(paste(Day,Month,Year,sep="/"), "%d/%m/%Y"),
         Week=as.numeric(format(date_raw, "%V"))) %>% rename(ID=ID_Simple)
...
list_files_field_level <- files_base[grepl(paste("2020_Plot_",Plot_i,sep = ""), files_base)]
...
S_edge_list_ID_fil <- S_edge_list_ID
...
write_csv(plot_modules_i,paste0("Processed_data/Modularity_Pheno_Overlap/2020_Modularity_Plot",
                                Plot_i,".csv")  )

}

write_csv(S_edge_list_final,"Processed_data/Modularity_Pheno_Overlap/2020_Edge_list_Phen_Over_PLOT.csv")
