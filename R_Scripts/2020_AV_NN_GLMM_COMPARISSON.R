...
visreg(GF_MIX_LIN_Visits, "visits_GF", by="Plant_Simple",gg = TRUE, overlay=F, partial=FALSE, rug=FALSE)+
  ...
  labs(x ="visits_GF", y = "Ln(Seeds per individual)",fill=NULL,color=NULL)

visreg(GF_MIX_LIN_Visits, "visits_GF", by="Plot",gg = TRUE, overlay=F, partial=FALSE, rug=FALSE)+
  theme_bw() +
  #geom_point(aes(color=G_F), size=2, alpha=0.3, shape=16)+
  geom_point(size=1.5, alpha=0.2, shape=16)+
...
# Checking Residuals 
plot(res_GF_MIX_NB_intercept_Plot_Plant) #KS +3 Quant desv
plot(res_GF_MIX_NB_intercept_Plot_Plant_ZI) #KS +3 Quant desv
plot(res_GF_MIX_LIN_intercept_Plot_Plant) #KS +3 Quant desv
plot(res_GF_MIX_LIN_intercept_Plot_Plant_ZI) #KS +3 Quant desv
...
# Checking Residuals 
plot(res_GF_MIX_NB_type_Plot) #KS +3 Quant desv
plot(res_GF_MIX_NB_type_Plot) #KS +3 Quant desv
...
GF_PUPA_LIN_antag_Plot <- glmmTMB(log(Seeds_GF) ~ #scale(homo_motif) +  #Only one visit
                                    #scale(hete_motif) +
                                    (1|Plot),
                                  #ziformula = ~1,
                                  family = gaussian(),
