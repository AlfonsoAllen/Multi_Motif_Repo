...
# Centrality metrics have been estimated by using pollinators efficiency 
...
fitness_final_aux <- read.csv(file = "NN_ALPHA_data_models_phenol_overlap.csv",
                         header = TRUE,
...
# Observations without visits
nrow(fitness.data %>% filter(ID!="None"))
...
summary(scale(fitness_PUPA$hete_motif)) #There are no hete-motifs
...
summary(GF_MIX_LIN_type_Plot) #Very similar results
...
# Checking Residuals 
plot(res_GF_MIX_NB_type_Plot) #KS + 3 Q deviations
plot(res_GF_MIX_LIN_type_Plot)#KS + 3 Q deviations
...
summary(GF_MIX_LIN_other_Plot) # There is signal in both homo+hete-motifs
...
summary(GF_PUPA_LIN_intercept_Plot) #strength
summary(GF_CHFU_LIN_intercept_Plot) #homo+hetemotifs
...
#Some variance components equal zero (Random factor)
...
plot(res_GF_CHFU_LIN_type_Plot) # KS + 3 Quantile deviations IT seems that there is overdisp.
# A Neg. Bin. model should fix that
...
summary(GF_CHFU_LIN_mutua_Plot) 
...
testDispersion(res_GF_CHFU_LIN_other_Plot)
