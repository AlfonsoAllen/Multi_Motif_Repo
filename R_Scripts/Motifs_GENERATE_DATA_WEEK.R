...
for (week_i in unique(fitness2$Week)){
  ...
  #fitness_week_i <- fitness_week_i %>% group_by(Plot,Subplot,Plant_Simple,G_F,num.plantas,Fruit,Seed) %>%
  ...
  fitness_week_i <- fitness_week_i %>% group_by(Plot,Subplot,Plant_Simple,G_F) %>%
    count(wt=Visits) %>% rename(Visits_tot = n)
  
  fitness_week_i$Subplot_Plant_Label <- paste(fitness_week_i$Subplot,fitness_week_i$Plant_Simple,sep = " ")
  fitness_week_i <- fitness_week_i
  
  visit_list_week <- fitness_week_i %>% ungroup() %>% select(Plot,G_F,Subplot_Plant_Label,Visits_tot)
