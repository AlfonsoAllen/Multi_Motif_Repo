...
cz_edge_module_info_IN <- read_csv("Processed_data/cz_edge_module_info_IN.csv") %>%
  dplyr::select(Plot,node_to,c,z) %>% separate(node_to,c("Subplot","Plant_Simple")," ")
cz_edge_module_info_IN$Plot <- factor(cz_edge_module_info_IN$Plot)

fitness.data.cz <- fitness.data %>%
  left_join(cz_edge_module_info_IN, by = c("Plot","Subplot","Plant_Simple"))

fitness.data.cz$c[is.na(fitness.data.cz$c)] <- 0
fitness.data.cz$z[is.na(fitness.data.cz$z)] <- 0

fitness_orig.cz <- fitness_orig %>%
  left_join(cz_edge_module_info_IN, by = c("Plot","Subplot","Plant_Simple"))

fitness_orig.cz$c[is.na(fitness_orig.cz$c)] <- 0
fitness_orig.cz$z[is.na(fitness_orig.cz$z)] <- 0
...
fitness_orig_LEMA <- fitness_orig.cz %>% filter(Plant_Simple=="LEMA")
fitness.data_LEMA <- fitness_orig.cz %>% filter(Plant_Simple=="LEMA")
...
fitness_orig_CHFU <- fitness_orig.cz %>% filter(Plant_Simple=="CHFU")
fitness.data_CHFU <- fitness.data.cz %>% filter(Plant_Simple=="CHFU")
...
fitness_orig_PUPA <- fitness_orig.cz %>% filter(Plant_Simple=="PUPA")
fitness.data_PUPA <- fitness.data.cz %>% filter(Plant_Simple=="PUPA")
