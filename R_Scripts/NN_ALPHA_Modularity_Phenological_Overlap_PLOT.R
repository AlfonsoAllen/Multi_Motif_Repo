...
alpha_factor_list$alpha <- NA

mutualist_list_A <- c("Bees")
mutualist_list_B <- c("Bees/sirphid","Hoverflies")
antagonist_list <- c("Big_beetles","Flower_beetles")

for(i in 1:nrow(alpha_factor_list)){
  
  if (alpha_factor_list$G_F[i] %in% mutualist_list_A){
    alpha_factor_list$alpha[i]=1.0
  }else if(alpha_factor_list$G_F[i] %in% mutualist_list_B){
    alpha_factor_list$alpha[i]=0.6
  }else if(alpha_factor_list$G_F[i] %in% antagonist_list){
    alpha_factor_list$alpha[i]=0.1
  }else{alpha_factor_list$alpha[i]=0.3}

}

alpha_factor_list <- alpha_factor_list %>% dplyr::select(-G_F) %>%
  rename(ID=ID_Simple)
...
###############
# Apply pollination factor alpha to weights

alpha_factor_list_from <- rename(alpha_factor_list,node_from=ID)
alpha_factor_list_to <- rename(alpha_factor_list,node_to=ID)
S_edge_list_aux <- S_edge_list %>% left_join(alpha_factor_list_from,by="node_from") %>%
  left_join(alpha_factor_list_to,by="node_to")

S_edge_list_aux$weight_coeff <- NA
S_edge_list_aux$alpha_coeff <- NA

for (i in 1:nrow(S_edge_list_aux)){
  
  S_edge_list_aux$alpha_coeff[i] <- sum(S_edge_list_aux$alpha.x[i],S_edge_list_aux$alpha.y[i], na.rm=TRUE)
  S_edge_list_aux$weight_coeff[i] <- S_edge_list_aux$alpha_coeff[i]*S_edge_list_aux$weight[i]
}

S_edge_list$weight <- S_edge_list_aux$weight_coeff
...
write_csv(plot_modules_NN_i,paste0("Processed_data/Modularity_Pheno_Overlap/NN_ALPHA_Modularity_Plot",
                                   Plot_i,".csv")  )

}

write_csv(centrality_final,"NN_ALPHA_PageRank_results.csv")
