...
motifs_extraction_Line <- function(visit_list) {
  ...
  for (line_i in 1:length(list_incid_matrix_19)){
    ...
    incid_matrix_i <- list_incid_matrix_19[[line_i ]] 
    ...
    if (length(motifs)>0){
      ...
      tbl_colnames <- c("line_id","number_nodes","number_plants","plant_1","subplot_plant_1",
                        "plant_2","subplot_plant_2","poll_1","poll_2")
      
      motif_3 <- as_tibble(data.frame(matrix(nrow=length(motifs),ncol=length(tbl_colnames))))
      colnames(motif_3) <- tbl_colnames
      
      motif_3$line_id <- as.numeric(names_line[line_i])
      ...
      for (i in 1:length(motifs)){
        ...
        for (j in 1:length(lista_names_i)) {
          ...
          if (lista_names_i[j] %in%  visit_list$Subplot_Plant_Label){
            ...
            list_plants_loc <- c(list_plants_loc,c(paste0(location_plant[[1]][1]," ",location_plant[[1]][2])))
            list_plants <- c(list_plants,c(location_plant[[1]][3]))
      ...
      print(paste("Triplets_plot",names_line[line_i],sep=""))
      #write_csv(motif_3, paste("Triplets 3_plot",plot_i,".csv",sep=""))
      
      if (line_i==fist_plot_i){
...
homo_hete_motifs <- function(visit_list) {
  ...
  motif_3_Carac <- motifs_extraction_Line(visit_list)
  ...
  for (i in 1:nrow(motif_3_Carac)){
    ...
    motif_3_Carac$descript_plant_1[i] <- paste(motif_3_Carac$line_id[i],
                                                motif_3_Carac$subplot_plant_1[i],
                                                motif_3_Carac$plant_1[i],
                                                sep = " ")
  
    if (!is.na(motif_3_Carac$plant_2[i])){
      motif_3_Carac$descript_plant_2[i] <- paste(motif_3_Carac$line_id[i],
  ...
  for (i in 1:nrow(output_funct)){
    ...
    descrip <-  paste(output_funct$Line[i],
                      output_funct$Subplot_Plant_Label[i],sep = " ")
...
for (week_i in unique(fitness2$Week)){
  ...
  fitness_week_i <- fitness_week_i %>% group_by(Plot,Subplot,Plant_Simple,ID,Line) %>%
    count(wt=Visits) %>% rename(Visits_tot = n)
  
  fitness_week_i$Subplot_Plant_Label <- paste(fitness_week_i$Plot,
                                              fitness_week_i$Subplot,
                                              fitness_week_i$Plant_Simple,sep = " ")
  fitness_week_i <- fitness_week_i
  
  visit_list_week <- fitness_week_i %>% ungroup() %>% select(Line,ID,Subplot_Plant_Label,Visits_tot)
  
  visit_list_week <- homo_hete_motifs(visit_list_week)
  visit_list_week <- visit_list_week %>% mutate(Week=week_i)
  
  visit_list_week <- visit_list_week %>% separate(Subplot_Plant_Label,
                                                  c("Plot","Subplot","Plant_Simple")," ")
...
write_csv(visit_list, "Processed_data/Motifs_WEEK/Caracoles_WEEK_SPECIES_LINE.csv")
