...
for (i in 1:9){
  ...
  file_i = paste0("Processed_data/Muxviz_Pheno_Overlap/2020_centrality_table_Plot",i)
  Centrality_i <- read_delim(file_i,";")
  
  # Remove data for eigenvectors
  Centrality_i <- Centrality_i %>% filter(Layer=="1-Multi") %>% mutate(Plot=i) %>%
    dplyr::select(-Layer,-Node,-Eigenvector) %>% separate(Label,c("Subplot","Plant")," ")
...
write_csv(fitness_final,"2020_data_models_phenol_overlap.csv")
