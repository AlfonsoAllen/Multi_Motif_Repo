...
pollination_20_i <- pollination_20_i %>% 
  select(Day,Month,Year,Line,Plot,Subplot,Plant,ID_Simple,Visits) %>% rename(ID=ID_Simple) %>%
...
plant_pheno_overlap <- function(plant1,plant2,pollination_20_i) {
  ...
  plants <- sort(unique(pollination_20_i$Plant))
...
newfolder <- paste("2020_Plot_",Plot_i,"/", sep="")
