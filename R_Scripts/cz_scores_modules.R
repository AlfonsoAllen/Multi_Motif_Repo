...
library(magrittr)
library(ggalluvial)
source("R_scripts/cz_values_function_OUT.R")
source("R_scripts/cz_values_function_IN.R")
...
edge_module_info_i <- S_edge_list %>% 
...
G_F_list_to <- read_csv("Raw_Data/Metadata_Pollinators_Abundances_Seeds_2019_ID.csv") %>%
...
G_F_list_to$G_F[G_F_list_to$node_to=="Odontomyia_sp."] <- "Small_flies"
G_F_list_to <- unique(G_F_list_to)

G_F_list_from$G_F[G_F_list_from$node_from=="Odontomyia_sp."] <- "Small_flies"
G_F_list_from <- unique(G_F_list_from)
###########

GF_cz_edge_module_info_IN <- cz_edge_module_info_IN %>% dplyr::left_join(G_F_list_to,by = "node_to")
GF_cz_edge_module_info_OUT <- cz_edge_module_info_OUT %>% dplyr::left_join(G_F_list_from,by = "node_from")

GF_cz_edge_module_info_IN$G_F[GF_cz_edge_module_info_IN$node_to=="Lassioglosum_.immunitum"] <- "Bees"
GF_cz_edge_module_info_OUT$G_F[GF_cz_edge_module_info_OUT$node_from=="Lassioglosum_.immunitum"] <- "Bees"
...
ggplot(GF_cz_edge_module_info_OUT%>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA")))+
...
GF_cz_edge_module_info_OUT %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA")) #449 plants
GF_cz_edge_module_info_OUT %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA")) #218 visitors 

#HUBS
GF_cz_edge_module_info_OUT %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) #0 plants
GF_cz_edge_module_info_OUT %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) #6 animal

GF_cz_edge_module_info_OUT %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6)%>%
  group_by(node_from, layer_from,Plot,G_F) %>% count()

#Perip
GF_cz_edge_module_info_OUT %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<.6) #3 plants
GF_cz_edge_module_info_OUT %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<.6)%>%
  group_by(layer_from,Plot) %>% count() %>% arrange(Plot) #24 plants


GF_cz_edge_module_info_OUT %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<.6) #5 animal


#Perip
GF_cz_edge_module_info_OUT %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),
                                     c>.6,z<2.6) #408 plants
GF_cz_edge_module_info_OUT %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),
                                     c>.6,z<2.6) #184 animal




GF_cz_edge_module_info_OUT%>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) %>% select(node_from,Plot) %>%
  unique()
# Periferical plants

GF_cz_edge_module_info_OUT%>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<0.6) %>% select(node_from,Plot) %>%
  unique()

# Network HUBS
GF_cz_edge_module_info_OUT%>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) %>% select(node_from,layer_from,Plot) %>%
  unique()

# Periphery
GF_cz_edge_module_info_OUT%>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<0.6) %>% select(node_from,layer_from,Plot) %>%
  unique()
...
ggplot(GF_cz_edge_module_info_IN%>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA")))+
  geom_point(aes(x=c,y=z,color=G_F),position = "jitter",alpha=.5)+
  geom_vline(xintercept = 0.6)+
  geom_hline(yintercept = 2.6)+
  theme_bw()+
...
GF_cz_edge_module_info_IN %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA")) #449 plants
GF_cz_edge_module_info_IN %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA")) #218 visitors 

#HUBS
GF_cz_edge_module_info_IN %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) #0 plants
GF_cz_edge_module_info_IN %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) #14 animal

GF_cz_edge_module_info_IN %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6)%>%
  group_by(node_to, layer_to,Plot,G_F) %>% count()

#Perip
GF_cz_edge_module_info_IN %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<.6) #24 plants
GF_cz_edge_module_info_IN %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<.6)%>%
  group_by(layer_to,Plot) %>% count() %>% arrange(Plot) #24 plants

GF_cz_edge_module_info_IN %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<.6) #3 animal


#Perip
GF_cz_edge_module_info_IN %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),
                                     c>.6,z<2.6) #409 plants
GF_cz_edge_module_info_IN %>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),
                                     c>.6,z<2.6) #176 animal


GF_cz_edge_module_info_IN %>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) %>% select(node_to,Plot) %>%
  unique()

GF_cz_edge_module_info_IN%>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<0.6) %>% select(layer_to,Plot) %>%
  group_by(layer_to,Plot) %>% count() 

#Network HUBS
GF_cz_edge_module_info_IN%>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) %>% select(node_to,layer_to,Plot) %>%
  unique()

GF_cz_edge_module_info_IN%>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z<2.6,c>0.6) %>% select(node_to,layer_to,Plot) %>%
  unique()

HUBS_pol_IN <- GF_cz_edge_module_info_IN%>% filter(G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),z>2.6) %>% select(node_to,layer_to,Plot) %>%
  unique()


GF_cz_edge_module_info_IN%>% filter(!G_F %in% c("LEMA","CHFU","CHMI","ME","PUPA"),c<0.6) %>% select(node_to,layer_to,Plot) %>%
  unique()
