...
GF_all_sp <- lm(log(Seeds_GF) ~ scale(homo_motif) +
                       scale(hete_motif) + 
...
GF_all_sp_ZI <- lm(log(Seeds_GF) ~ scale(homo_motif) + 
                       scale(hete_motif) + 
                        scale(StrengthIn) + scale(Ratio) +
                       Plant_Simple + 
                       G_F+ 
...
GF_all_sp_GF <- lm(log(Seeds_GF) ~ scale(homo_motif) + 
                       scale(hete_motif) + 
                      scale(StrengthIn) + scale(Ratio) + 
                       Plant_Simple + 
                       G_F,
...
GF_all_sp_Plot <- lm(log(Seeds_GF) ~ scale(homo_motif) + 
                            scale(hete_motif) + 
                            scale(StrengthIn) + scale(Ratio) + 
                            Plant_Simple + 
                            Plot,
...
GF_all_sp_Plant <-lm(log(Seeds_GF) ~ scale(homo_motif) +
                            scale(hete_motif) + 
                             scale(StrengthIn) + scale(Ratio) + 
                            Plant_Simple,
                          family = gaussian(),
...
testSpatialAutocorrelation(res_all, x = fitness.data$x, y = fitness.data$y )
testSpatialAutocorrelation(res_all_ZI, x = fitness.data$x, y = fitness.data$y )
testSpatialAutocorrelation(res_all_plot, x = fitness.data$x, y = fitness.data$y )
testSpatialAutocorrelation(res_all_GF, x = fitness.data$x, y = fitness.data$y )
testSpatialAutocorrelation(res_all_plant, x = fitness.data$x, y = fitness.data$y )
...
performance::r2(GF_all_sp) #KS + heter.
performance::r2(GF_all_sp_ZI)#KS + heter.
performance::r2(GF_all_sp_Plot)#KS + heter.
performance::r2(GF_all_sp_GF) #KS + heter.
performance::r2(GF_all_sp_Plant)#KS + heter.
...
GF_LEMA_01 <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) +
                        scale(hete_motif) + 
                        scale(StrengthIn) + scale(Ratio) +
                        (0+scale(homo_motif)|G_F) + Plot,
                      family = gaussian(),
                      data = fitness_LEMA)

GF_LEMA_02 <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) + #No convergence
                        scale(hete_motif) + 
                        #scale(StrengthIn) +
                        scale(Ratio) +
                        (1 + scale(homo_motif)|ID) + Plot,
                      family = gaussian(),
                      data = fitness_LEMA)
...
GF_LEMA_01_ZI <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) +  #Convergence problems without degree
                           scale(hete_motif) + 
                           scale(StrengthIn) + scale(Ratio) + 
                           (0+scale(homo_motif)|G_F) + Plot,
                         ziformula = ~1,
...
plotResiduals(res_LEMA_01, (fitness_LEMA$Plot))
# 
...
LEMA_glm1 <- lm(log(Seeds_GF) ~ scale(homo_motif) + scale(hete_motif) + scale(StrengthIn) +  
                         scale(Ratio) + Plot, data = fitness_LEMA)
...
m_spamm <- fitme(log(Seeds_GF) ~ scale(homo_motif) +
                   scale(StrengthIn) + scale(Ratio) + 
                   Matern(1 | x + y) + Plot, data =fitness_PUPA, family = "gaussian") # this take a bit of time
# model summary
summary(m_spamm)
...
plotResiduals(res_LEMA_04, scale(fitness_LEMA$StrengthIn))#1 strong hete
plotResiduals(res_LEMA_04, scale(fitness_LEMA$Ratio)) #3 strong hete
...
plotResiduals(res_PUPA_04, scale(fitness_PUPA$StrengthIn))
plotResiduals(res_PUPA_04, scale(fitness_PUPA$Ratio)) 
...
plotResiduals(res_CHFU_04, scale(fitness_CHFU$StrengthIn))
plotResiduals(res_CHFU_04, scale(fitness_CHFU$Ratio)) 
