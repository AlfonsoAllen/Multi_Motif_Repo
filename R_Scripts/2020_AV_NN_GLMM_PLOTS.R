...
seed_data <- seed_data_raw %>% 
  dplyr::select(Plot,Subplot,Plant,`Mean Seeds/Fruit`,`Mean Fruits/panicle`) %>% unique() %>%
  rename(Seed=`Mean Seeds/Fruit`,Fruit=`Mean Fruits/panicle`) %>% group_by(Plot,Subplot,Plant) %>%
...
write_csv(fitness_final,"2020_AV_NN_data_models_phenol_overlap.csv")
