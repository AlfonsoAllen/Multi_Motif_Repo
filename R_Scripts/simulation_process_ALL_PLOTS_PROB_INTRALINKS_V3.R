...
# Load MOTIF data
raw_intralinks_MOTIFS <- read_csv("Processed_data/Data_simulation/data_changing_INTRALINKS_MOTIFS_V3.csv")
...
raw_intralinks_PROBABILITY <- read_csv("Processed_data/Data_simulation/data_changing_INTRALINKS_PROBABILITIES_V3.csv")
raw_intralinks_PROBABILITY_UNCOUPLED <- read_csv("Processed_data/Data_simulation/data_changing_INTRALINKS_PROBABILITIES_UNCOUPLED_V3.csv")
...
total_homo_MOTIFS_deco <- bind_rows(total_homo_intralinks_MOTIFS_deco)

plot_homo <- ggplot(total_homo_MOTIFS_deco,
                       aes(x=number_intra/number_individuals, y=homo_motif, group=number_intra/number_individuals))+
  ...
  labs(y="Number of homospecific subgraphs",x="Intralink density")+
...
total_hete_MOTIFS_deco <- bind_rows(total_hete_intralinks_MOTIFS_deco)

plot_hete <- ggplot(total_hete_MOTIFS_deco,
                    aes(x=number_intra/number_individuals, y=hete_motif, group=number_intra/number_individuals))+
  # geom_jitter(aes(color=as.factor(number_intra)),shape=16, position=position_jitter(9), alpha=0.3)+
  geom_boxplot(alpha = 1.0,lwd=0.7)+
  stat_summary(fun = mean, color = "red", geom = "line", aes(group = 1),lwd=1.5)+
  stat_summary(fun =mean, geom="point", shape=23, size=3,fill="red")+
  ...
  scale_color_brewer(palette = "Set2")+theme_bw()+
  theme(legend.position="none", plot.title = element_text(size = 11))+
  labs(y="Number of heterospecific subgraphs",x="Intralink density")+
  theme(axis.text=element_text(size=15),  axis.title=element_text(size=17,face="bold"))+                                                                # Change font size
  theme(strip.text.x = element_text(size = 18))+
  guides(fill=guide_legend(nrow=2,byrow=TRUE),colour = guide_legend(override.aes = list(size=5)))
...
total_heter_prob_PROBABILITY_deco <- bind_rows(
                                    total_heter_prob_intralinks_PROBABILITY_deco
)

plot_heter_prob <- ggplot(total_heter_prob_PROBABILITY_deco,
                    aes(x=number_intra/number_individuals, y=heter_prob, group=number_intra/number_individuals))+
  # geom_jitter(aes(color=as.factor(number_intra)),shape=16, position=position_jitter(9), alpha=0.3)+
  geom_boxplot(alpha = 1.0,lwd=0.7)+
  stat_summary(fun = mean, color = "red", geom = "line", aes(group = 1),lwd=1.5)+
  stat_summary(fun =mean, geom="point", shape=23, size=3,fill="red")+
  ...
  scale_color_brewer(palette = "Set2")+theme_bw()+
  theme(legend.position="none", plot.title = element_text(size = 11))+
  labs(y="Prob. of receiving pollen\nfrom heterosp.",x="Intralink density")+
  theme(axis.text=element_text(size=15),  axis.title=element_text(size=17,face="bold"))+                                                                # Change font size
  theme(strip.text.x = element_text(size = 18))+
  guides(fill=guide_legend(nrow=2,byrow=TRUE),colour = guide_legend(override.aes = list(size=5)))
...
total_consp_prob_UNCOUPLED_PROBABILITY_deco <- bind_rows(
                                               total_consp_prob_UNCOUPLED_intralinks_PROBABILITY_deco
)

plot_consp_prob_UNCOUPLED <- ggplot(total_consp_prob_UNCOUPLED_PROBABILITY_deco,
                          aes(x=number_intra/number_individuals, y=consp_prob_UNCOUPLED, group=number_intra/number_individuals))+
  # geom_jitter(aes(color=as.factor(number_intra)),shape=16, position=position_jitter(9), alpha=0.3)+
  geom_boxplot(alpha = 1.0,lwd=0.7)+
  stat_summary(fun = mean, color = "red", geom = "line", aes(group = 1),lwd=1.5)+
  stat_summary(fun =mean, geom="point", shape=23, size=3,fill="red")+
  ...
  scale_color_brewer(palette = "Set2")+theme_bw()+
  theme(legend.position="none", plot.title = element_text(size = 11))+
  labs(y="Prob. of receiving pollen\nfrom consp. (uncopled layers)",x="Intralink density")+
...
#--------
library(patchwork)
png("New_Figures/simulations_intralinks_boxplot_V3.png",
...
amod <- aov(consp_prob_UNCOUPLED ~ condition, total_consp_prob_UNCOUPLED_PROBABILITY_deco %>%
              mutate(condition=as.factor(number_intra/number_individuals)))
...
png("New_Figures/pairwise_comp_consp_prob_UNCOUPLED_intralinks_V3.png",
...
# results_consp <- tibble(comparisson = names(coefficients),
...
png("New_Figures/pairwise_comp_heter_prob_intralinks_V3.png",
    width = 11.69*0.55, # The width of the plot in inches
    height = 11.69*0.35, units = "in", res=300*2)
