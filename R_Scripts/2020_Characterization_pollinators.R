...
ggplot(fitness_orig %>% filter(G_F!="None"),aes(x=(G_F),y=scale(Seeds_GF),fill=G_F))+
  ...
  facet_wrap(vars(Plant),nrow = 3,ncol = 3)+
...
ggplot(fitness.data,aes(x=(homo_motif),y=entropy,color=Plant))+
  geom_point()+geom_smooth(method = 'lm',formula = y~x)

ggplot(fitness.data%>% filter(entropy>0),aes(x=(homo_motif),y=entropy,color=Plant))+
  geom_boxplot()+facet_wrap(vars(Plot),nrow = 3,ncol = 3)
ggplot(fitness.data,aes(x=as.factor(homo_motif),y=entropy,color=Plant))+
  geom_boxplot()+facet_wrap(vars(Plot),nrow = 3,ncol = 3)

ggplot(fitness.data,aes(x=as.factor(homo_motif),y=entropy,color=Plant))+
  geom_violin()

ggplot(fitness.data,aes(x=Real_PR_Multi,y=entropy,color=Plant))+
  geom_point()+geom_smooth(method = 'lm',formula = y~x)+
  facet_wrap(vars(Plot),nrow = 3,ncol = 3)

ggplot(fitness.data,aes(x=Real_PR_Multi,y=entropy,color=Plant))+
  geom_point()+geom_smooth(method = 'lm',formula = y~x)
ggplot(fitness.data,aes(x=Real_PR_Multi,y=homo_motif,color=Plant))+
  geom_point()+geom_smooth(method = 'lm',formula = y~x)+
  facet_wrap(vars(Plot),nrow = 3,ncol = 3)
...
ggplot(fitness.data,aes(x=Real_PR_Multi,y=log(Seeds_GF),color=Plant))+
  geom_point()+geom_smooth(method = 'lm',formula = y~x)+
  facet_wrap(vars(Plant),nrow = 3,ncol = 3)

ggplot(fitness.data,aes(x=homo_motif,y=log(Seeds_GF),color=Plant))+
  geom_point()+geom_smooth(method = 'lm',formula = y~x)+
