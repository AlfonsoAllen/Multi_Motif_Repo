...
plot_multilayer <- function(plot_id){
  # Load multilayer----------
  ...
  r_plants <- 1
  r_pollinators <- 0.5*(max(nodes_prop_pollinator_LEMA$x)-min(nodes_prop_pollinator_LEMA$x))
  
  
  if(nrow(nodes_prop_plant_OTHER)>1){
    alpha_plants <- pi/(nrow(nodes_prop_plant_OTHER)-1)
    
    nodes_prop_plant <- nodes_prop_plant_OTHER %>%
      mutate(row_number = as.numeric(rownames(nodes_prop_plant_OTHER)),
    ...
    nodes_prop_plant <- nodes_prop_plant_OTHER %>%
      mutate(x = 0,
             y = r_plants) %>% select(name, x, y)
  }
  
  if(nrow(nodes_prop_pollinator_OTHER)>1){
    
    alpha_pollinators <- pi/(nrow(nodes_prop_pollinator_OTHER)-1)
    
    nodes_prop_pollinator <- nodes_prop_pollinator_OTHER %>%
      mutate(row_number = as.numeric(rownames(nodes_prop_pollinator_OTHER)),
...
#par(mfrow=c(3,3))
par(mar=c(0,0,0,0)+.1)
multi_1 <- plot_multilayer(1)
multi_2 <- plot_multilayer(2)
multi_3 <- plot_multilayer(3)
multi_4 <- plot_multilayer(4)
multi_5 <- plot_multilayer(5)
multi_6 <- plot_multilayer(6)
multi_7 <- plot_multilayer(7)
multi_8 <- plot_multilayer(8)
multi_9 <- plot_multilayer(9)
