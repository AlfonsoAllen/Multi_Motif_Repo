...
GF_all_sp <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) +
                       scale(hete_motif) + 
...
GF_all_sp_ZI <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) + 
                       scale(hete_motif) + 
                        scale(StrengthIn) + scale(Ratio) +
                       (1 | Plant_Simple) + 
                       (1 | G_F)+ 
...
GF_all_sp_GF <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) + 
                       scale(hete_motif) + 
                      scale(StrengthIn) + scale(Ratio) + 
                       (1 | Plant_Simple) + 
                       (1 | G_F),
...
GF_all_sp_Plot <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) + 
                            scale(hete_motif) + 
                            scale(StrengthIn) + scale(Ratio) + 
                            (1 | Plant_Simple) + 
                            (1 | Plot),
...
GF_all_sp_Plant <- glmmTMB(log(Seeds_GF) ~ scale(homo_motif) +
                            scale(hete_motif) + 
                             scale(StrengthIn) + scale(Ratio) + 
                            (1 | Plant_Simple),
                          family = gaussian(),
...
plot(res_PUPA_01) #het
plot(res_PUPA_02) #het
plot(res_PUPA_both)
plot(res_PUPA_03)
...
v <- visreg(GF_LEMA_01, "homo_motif", by="G_F")
plot(v)
...
LEMA_by <- brm(log(Seeds_GF) ~ scale(homo_motif) +
                 scale(hete_motif) + 
                 scale(StrengthIn) + scale(Ratio), data = fitness_LEMA,
...
CHFU_by <- brm(log(Seeds_GF) ~ scale(homo_motif) +
                 scale(hete_motif) + 
                 scale(StrengthIn) + scale(Ratio), data = fitness_CHFU,
               family = negbinomial(link = "log"))
summary(CHFU_by)
r2(CHFU_by)
r2(CHFU_glm1)
